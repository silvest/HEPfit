...
double tauKnu_o_tauPnu::computeThValue()
{
    ...
    gslpp::vector<gslpp::complex> ** allcoeff_tau_K_nu = SM.getFlavour().ComputeCoeffdiujlknu(1,0,2,mtau);
    gslpp::vector<gslpp::complex> ** allcoeff_tau_P_nu = SM.getFlavour().ComputeCoeffdiujlknu(0,0,2,mtau);
    ...
    double num = ((*(allcoeff_tau_K_nu[LO]))(0) -(*(allcoeff_tau_K_nu[LO]))(1) + mK * mK / (ms+mu) / mtau * ((*(allcoeff_tau_K_nu[LO]))(2) - (*(allcoeff_tau_K_nu[LO]))(3))).abs2();
    double den = ((*(allcoeff_tau_P_nu[LO]))(0) -(*(allcoeff_tau_P_nu[LO]))(1) + mP * mP / (md+mu) / mtau * ((*(allcoeff_tau_P_nu[LO]))(2) - (*(allcoeff_tau_P_nu[LO]))(3))).abs2();
    
    //double WCnum = 1.0;
    //double WCden = 1.0;
   
    //return 1.;
    return (1-mK*mK/mtau/mtau) * (1-mK*mK/mtau/mtau)/(1-mP*mP/mtau/mtau)/(1-mP*mP/mtau/mtau)*fKofP*fKofP*(1+deltauK2tauP2em)*num/den;
