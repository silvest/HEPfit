...
: ThObservable(SM_i)
{
    ...
    setParametersForObservable(make_vector<std::string>() << "fP" << "delPl2em");
...
double Pmunu::computeThValue()
{
    ...
    double fP = SM.getOptionalParameter("fP"); // this is the charged pion decay constant in the SU(2)-symmetric limit 
    ...
    //return 1.;
    return 1. / (64. * M_PI) * mmu * mmu * mP * (1-mmu*mmu/mP/mP) * (1-mmu*mmu/mP/mP) * fP * fP * (1+delPl2em) * amp2;
