...
MPll::MPll(const StandardModel& SM_i, StandardModel::lepton lep_i) : ThObservable(SM_i), mySM(SM_i)
        /*,H_V0cache(2, 0.),
        H_V1cache(2, 0.),
        H_V2cache(2, 0.),
        H_Scache(2, 0.),
...
/*void MPll::checkCache(double qmin, double qmax){
    ...
    if (qmax < CUTOFF) {
        ...
        if (a_0A12 == VL0_cache(0) && a_1A12 == VL0_cache(1) && MK == VL0_cache(2) ){
...
double MPll::z(double q2){
    double t_0 = 12.;
    double t_p=pow(MB + MK,2.);
    return ( sqrt(t_p - q2) - sqrt(t_p - t_0) ) / ( sqrt(t_p - q2) + sqrt(t_p - t_0) );
}



double MPll::lat_fit(double q2, double a_0, double a_1, double dm){
    return 1 / (1 - q2/pow(MB + dm,2.)) * ( a_0 + a_1*z(q2) );
}



double MPll::V(double q2){
    if (q2<CUTOFF)
...
double MPll::A_2(double q2){
    return LCSR_fit2(q2, r_1A2, r_2A2, m_fit2A2);
}



double MPll::T_1(double q2){
    if (q2<CUTOFF)
...
double MPll::T_3tilde(double q2){
    return LCSR_fit2(q2, r_1T3t, r_2T3t, m_fit2T3t);
}



double MPll::T_3(double q2){
    if (q2 < 2.) return (0.178168 - 0.202)/2. * q2 + 0.202;
    else return (MB*MB - MK*MK)/q2*(T_3tilde(q2) - T_2(q2));
}



double MPll::V_L(int i, double q2){
    switch (i){
        case 0:
            if (q2 < CUTOFF)
                return 1. / ( 4.*MK*MB*(MB + MK)*sqrt(q2) ) * ( pow((MB + MK),2.)*(MB*MB - q2 - MK*MK)*A_1(q2) - lambda(q2)*A_2(q2) );
            else
                return 4*MK/sqrt(q2)*lat_fit(q2, a_0A12, a_1A12, dmA12);
        case 1:
            return 1./2. * ( ( 1. + MK/MB)*A_1(q2) - sqrt(lambda(q2))/ ( MB* (MB + MK) ) * V(q2) );
        case 2:
            return 1./2. * ( ( 1. + MK/MB)*A_1(q2) + sqrt(lambda(q2))/ ( MB* (MB + MK) ) * V(q2) );
        default:
...
double MPll::V_R(int i, double q2){
    if (i != 0) i=3-i;
    return -V_L(i,q2);
}
...
double MPll::T_L(int i, double q2){
    switch (i){
        case 0:
            if (q2 < CUTOFF)
                return sqrt(q2)/(4.*MB*MB*MK) * ( ( MB*MB+ 3.*MK*MK - q2 ) * T_2(q2) - lambda(q2) / (MB*MB - MK*MK) * T_3(q2) );
            else
                return 2*sqrt(q2)*MK/MB/(MB + MK)*lat_fit(q2, a_0T23, a_1T23, dmT23);
        case 1:
            return (MB*MB - MK*MK) / ( 2.*MB*MB ) * T_2(q2) - sqrt(lambda(q2)) / ( 2.*MB*MB ) * T_1(q2);
        case 2:
            return (MB*MB - MK*MK) / ( 2.*MB*MB ) * T_2(q2) + sqrt(lambda(q2)) / ( 2.*MB*MB ) * T_1(q2);
        default:
            std::stringstream out;
...
double MPll::T_R(int i, double q2){
    if (i != 0) i=3-i;
    return -T_L(i,q2);
}
...
gslpp::complex MPll::H_V(int i, double q2, int bar) {
    gslpp::complex n;
    switch(bar){
        case 0:
            n = N();
...
gslpp::complex MPll::H_A(int i, double q2, int bar) {
    gslpp::complex n;
    switch(bar){
        case 0:
            n = N();
...
gslpp::complex MPll::H_P(double q2, int bar) {
    ...
    return gslpp::complex::i()*n*( Mb/MW*( (*(allcoeff[LO]) + *(allcoeff[NLO]))(11)*S_L(q2) 
            ...
            + 2.*Mlep*Mb/q2*( (*(allcoeff[LO]) + *(allcoeff[NLO]))(9)*( S_L(q2) - Ms/Mb*S_R(q2) ) 
...
double MPll::F(double q2, double b_i) {
...
GammaPrime::GammaPrime(const StandardModel& SM_i, StandardModel::lepton lep_i) : MPll(SM_i, lep_i) {  
}
...
double GammaPrime::computeGammaPrime(double qmin, double qmax) {
    ...
    F1 = convertToGslFunction( boost::bind( &MPll::getSigma2, &(*this), _1 ) );
    ...
    F2 = convertToGslFunction( boost::bind( &MPll::getSigma3, &(*this), _1 ) );
    ...
    F3 = convertToGslFunction( boost::bind( &MPll::getSigma0, &(*this), _1 ) );
    ...
    F4 = convertToGslFunction( boost::bind( &MPll::getSigma1, &(*this), _1 ) );
...
double A_FB::computeThValue() {
    ...
    return -3. * avaSigma7 / 4. / computeGammaPrime(q_min, q_max);
}


BR_MPll::BR_MPll(const StandardModel& SM_i, StandardModel::lepton lep_i) : GammaPrime(SM_i, lep_i) {  
}
...
double ACP::computeThValue() {
    ...
    return (3.*avaDelta0 - avaDelta2 + 2. * ( 3.*avaDelta1 - avaDelta3 ) )/(4.*computeGammaPrime(q_min, q_max));

}


P3CP::P3CP(const StandardModel& SM_i, StandardModel::lepton lep_i) : MPll(SM_i, lep_i) {  
}
...
double P3CP::computeThValue() {
    ...
    F1 = convertToGslFunction( boost::bind( &MPll::getDelta11, &(*this), _1 ) );
...
F_L::F_L(const StandardModel& SM_i, StandardModel::lepton lep_i) : MPll(SM_i, lep_i) {
}
...
double F_L::computeThValue() {
    ...
    return (3.*avaSigma0 - avaSigma2)/(((3.*avaSigma0 - avaSigma2) + 2.*(3.*avaSigma1 - avaSigma3)));
