...
bool HiggsKvKfgen::CheckParameters(const std::map<std::string, double>& DPars)
{
    for (int i = 0; i < NHKvKfgenvars; i++) {
        if (DPars.find(HKvKfgenvars[i]) == DPars.end()) {
            std::cout << "ERROR: missing mandatory HiggsKvKfgen parameter " << HKvKfgenvars[i] << std::endl;
...
double HiggsKvKfgen::mueeWBF(const double sqrt_s) const
{
...
double HiggsKvKfgen::BrHmumuRatio() const
{
...
double HiggsKvKfgen::computeKmu() const
{
    return Kl;
}
