...
#ifndef NPEFFECTIVEGIMRPRIME_H
#define	NPEFFECTIVEGIMRPRIME_H
 ...
 * (Use the model name "NPEffectiveGIMRprime_LFU_QFU" to asumme lepton and quark flavour universality)
 *
 ...
 * @f]
 * as written in the basis of \cite Grzadkowski:2010es , replacing the operators
 * \f${\cal O}_{HWB}=\big(H^\dagger\tau^a H\big)W_{\mu\nu}^a B^{\mu\nu}\f$ and 
 * \f${\cal O}_{HD}=\big|H^\dagger D_\mu H\big|^2\f$ by 
 * \f${\cal O}_{DHB}=i\big(D^\mu H^\dagger D^\nu H\big) B_{\mu\nu}\f$ and
 * \f${\cal O}_{DHW}=i\big(D^\mu H^\dagger \tau^a D^\nu H\big) W_{\mu\nu}^a\f$.
 * In this way only 8 dimension 6 operators contribute to electroweak precision
 * observables, the maximum number that can be constrained by the data.
 *
 ...
 * @f]
 *
 * Alternatively, when using the model name "NPEffectiveGIMRprime_LFU_QFU", where lepton
 ...
 * are the same for both "NPEffectiveGIMRprime" and "NPEffectiveGIMRprime_LFU_QFU".)
 * 
 ...
 * </table>
 *
 * @anchor NPEffectiveGIMRprimeFlags
 * <h3>%Model flags</h3>
 *
 ...
 *
 *
 * @anchor NPEffectiveGIMRprimeFunctions
 * <h3>Important member functions</h3>
 *
 * See the base classes of the current class.
 *
 */
class NPEffectiveGIMRprime : public NPbase {
public:
    ...
    /**
     * @brief The post-update method for %NPEffectiveGIMRprime.
    ...
    /**
     * @brief A method to check if all the mandatory parameters for %NPEffectiveGIMRprime
    ...
    /**
     * @brief A method to set a flag of %NPEffectiveGIMRprime.
