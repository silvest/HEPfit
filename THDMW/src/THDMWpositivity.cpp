...
double THDMWpositivity7::computeThValue()
{
    ...
    double mu5=myTHDMW.getTHDMW_mu5();
    double mu6=myTHDMW.getTHDMW_mu6();
    double muA=0.5*(mu1+mu2+mu6)+mu3+mu4+mu5;
    double nu1=myTHDMW.getTHDMW_nu1();
    double nu2=myTHDMW.getTHDMW_nu2();
    double nu3=myTHDMW.getTHDMW_nu3();
    double nu4=myTHDMW.getTHDMW_nu4();
    double nu5=myTHDMW.getTHDMW_nu5();
    if (lambda1>0 && muA>0)
    {
        return lambda1 + 0.5*muA + nu1 + nu2 + 2.0*nu3 - fabs(nu4+nu5)/sqrt(3.0);
    }
    else
    {
        return -1.;
...
THDMWpositivity12::THDMWpositivity12(const StandardModel& SM_i)
: ThObservable(SM_i),myTHDMW(static_cast<const THDMW&> (SM_i))
{}

double THDMWpositivity12::computeThValue()
{
    double lambda2=myTHDMW.getTHDMW_lambda2();
    double mu1=myTHDMW.getTHDMW_mu1();
    double mu2=myTHDMW.getTHDMW_mu2();
    ...
    double omega2=myTHDMW.getTHDMW_omega2();
    double omega3=myTHDMW.getTHDMW_omega3();
    double omega4=myTHDMW.getTHDMW_omega4();
    double omega5=myTHDMW.getTHDMW_omega5();
    if (lambda2>0 && muA>0)
    {
        return lambda2 + 0.5*muA + omega1 + omega2 + 2.0*omega3 - fabs(omega4+omega5)/sqrt(3.0);
    }
    else
    {
        return -1.;
