...
MonteCarlo::MonteCarlo(const char * name, Model * Mod_i,
        std::vector<ModelParameter> ModPars_i, std::vector<Observable> Obs_i,
...
BCModel(name) {
    ...
    Obs2D_ALL = Obs2D_i;
    int k = 0, kweight = 0;
    for (std::vector<Observable>::iterator it = Obs_i.begin();
            it < Obs_i.end(); it++) {
...
double MonteCarlo::Weight(const Observable& obs, const double& th) {
    double logprob;
...
double MonteCarlo::Weight(const Observable2D& obs, const double& th1, const double& th2) {
    double logprob;
...
void MonteCarlo::PrintHistogram(BCModelOutput& out) {
