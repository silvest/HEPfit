...
void MonteCarlo::generateEvent(const int rank, int unsigned nIteration, bool noMC, int seed)
{
    if(!noMC)
        throw std::runtime_error("\nMonteCarlo::generateEvent():\n--noMC was not specified as an argument.\nPlease do so for running in Generate Event mode.\n");
    if (checkMode == 1)
        throw std::runtime_error("\nMonteCarlo::generateEvent():\nMonte Carlo Mode requested first. Cannot run in Generate Event mode");
    checkMode = 1;
    try {
...
{
        if (checkMode == 1)
                throw std::runtime_error("\nMonteCarlo::Run():\nGenerate Event Mode requested first. Cannot run in Monte Carlo mode");
        checkMode = 1;
    try {
