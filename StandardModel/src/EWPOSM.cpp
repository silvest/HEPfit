...
double EWPOSM::sin2thwall(const std::string& ferm) const {
    ...
    double sin2t=sw2();
    if(i!=5) {sin2t= s0[i] + d1[i]*L_H + d2[i]*L_H*L_H + d3[i]*L_H*L_H*L_H*L_H
...
{sql=sqrt(sql);
if((M12!=M22) || std::abs(M12/Q2)>0.000001 ) {sql=1.0/sql*log((sql+Q2+M12+M22))-1.0/sql*log((-sql+Q2+M12+M22));}
else
...
if(Q2!=0.0){
            ...
            +1.0/2.0/Q2*l;}
else{if(M12==M22){i=log(M12/mW()/mW());}
    else{i= -log(mW()*mW())+1.0/(M12-M22)*log(M12/M22); }   }
return i;
}
...
{
if(Q2!=0.0){ i=0.25*log(M12*M22/mW()/mW()/mW()/mW())-1.0-(M12-M22)/2.0/Q2
                    -(2*M12/Q2+(M12-M22)*(M12-M22)/Q2/Q2)*log(M12/M22)
                    +(1.0+(M12-M22)/Q2)/4.0/Q2*l;}
        else
            {if(M12!=M22){i=0.25*(-1.0+2.0*M22/(M12-M22)-2.0*M22*M22*log(M12/M22)/(M12-M22)/(M12-M22)
                  +2.0*log(M12/mW()/mW())); }
            else{i=0.5*log(M12/mW()/mW());}
        }}
else{if(M12!=M22){
    i=(1.0/(120.0*pow((M12-M22),8.0)))*
        (-30.0*(M12 - 3*M22)*pow((M12 - M22),7.0) +
  20.0* pow((M12 - M22),5.0)*(M12*M12- 5*M12*M22 - 2*M22*M22)*Q2 - 
  5* pow((M12 - M22),3)*(pow(M12,3.0) - 11.0*M12*M12*M22 - 47.0*M12*M22*M22 -
     3.0* M22*M22*M22)*Q2*Q2 + 
  2.0* (pow(M12,5.0) - 20*pow(M12,4.0)*M22 - 220.0*pow(M12,3.0)*M22*M22 + 80.0*M12*M12*pow( M22,3.0) + 
     155.0*M12 *pow(M22,4) + 4*pow(M22,5))* pow(Q2,3) + 
  60.0* (-pow((M12 - M22),4)*M22*M22*(M12*M12 + M22*M22 - 
        2* M12 *(M22 + Q2)) *log(M12/
            M22) +
     M12*M22*M22*Q2*Q2 *(-3.0* pow(M12,3.0) + 2.0*M22*M22 *(-M22 + Q2) +
        4* pow(M12,2.0) *(M22 + Q2) + M12 *M22 *(M22 + 8.0* Q2))* log(
       M12)  +
     M12*M22*M22*Q2*Q2*((M12 - M22)*(M12-M22)* (3 *M12 + 2* M22) -
        2* (2*M12*M12 + 4*M12*M22 + M22*M22)*Q2)*log(M22)
        +pow((M12-M22),8.0)*log(M12/(mW()*mW()))
        ));}
else {i=(70.0*M12*M12*Q2 - 7*M12*Q2*Q2 + Q2*Q2*Q2 + 420.0*M12*M12*M12*log(M12/mW()/mW()))/(840.0 *M12*M12*M12);}}
}
else{if(Q2!=0.0){
    //auxiliary variable
...
gslpp::complex l(LL(Q2,M12x,M22x));
          if(Q2!=0.0) { i=log(M12x*M22x/mW()/mW()/mW()/mW())/12.0 -5.0/18.0
...
if((ferm=="up")||(ferm=="down")||ferm=="chlepton")
	{
	r=deltarhorem(ferm);
        gslpp::complex drmtot(deltarremtot());
	r=(1.0+r)/(1.0+deltarho()*(1.0-drmtot));
	}

else {std::cout
		<<"wrong input for fermion parameter,  it can be equal only to 'up','down', 'chlepton'   rhoZf"<<"\n";}
	return r;}
...
if((ferm=="up")||(ferm=="down")||ferm=="chlepton")
	{
     k=deltakrem(ferm);
    gslpp::complex drho(deltarho());
    gslpp::complex drrtot(deltarremtot());
    //std::cout<<k<<" "<<drho<<"\n";
    k=(1.0+k)*(1.0-cw2()/sw2()*drho*(1-drrtot));
  }
   else {std::cout
    <<"wrong input for fermion parameter "<< ferm<<"  it can be equal only to 'up','down', 'chlepton'"<<"\n";}
return k;}


/////////////////CHECK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//Equation 62 of GFitter
//Equation 2.4.23 ZFitter
gslpp::complex EWPOSM::deltarrem()
{
gslpp::complex d(deltarhowF());
d=(1.0/sw2()*(d+11.0/2.0-5.0/8.0*cw2()*(1+cw2())+9.0/4.0*cw2()/sw2()*log(cw2()))
-2.0/3.0+1.0/sw2()*(1.0/6.0*24.0-1.0/6.0-7.0*cw2())*log(cw2()))*sqrt(2)*GF*mZ*mZ*sw2()*cw2()/4.0/M_PI/M_PI;
	return d;}



//Equation (63)
gslpp::complex EWPOSM::deltarhowF()
{
gslpp::complex d(sigmaBFMw());
d=-d.real()+sigmaBF0()-sigmaFww(mW()*mW()).real()+sigmaFww(0.0);
return d;}

//Equation (64) of GFitter
//A.4.70 of ZFitter
gslpp::complex EWPOSM::deltarhorem(const std::string& ferm)
{
gslpp::complex d(0.0);
if((ferm=="up")||(ferm=="down")||ferm=="chlepton")
	{
	d=sigmaFzz1a();
        d=d+sigmaBFMz1()-deltarhozF()-11.0/2.0+5.0/8.0*cw2()*(1+cw2())-9.0/4.0*cw2()/sw2()*log(cw2())+2.0*uf(ferm);
          d=ale/4.0/M_PI/sw2()*d;
}
else {std::cout<<"wrong input for fermion parameter,  it can be equal only to 'up','down', 'chlepton' deltarhorem"<<"\n";}
return d;}


//Equation (65) GFItter
//Equation A.4.71 ZFitter
gslpp::complex EWPOSM::deltakrem(const std::string& ferm)
{
gslpp::complex k(0.0);
double qf;
if((ferm=="up")||(ferm=="down")||ferm=="chlepton")
{
	if(ferm=="up") qf=Qf[0];
	if(ferm=="down") qf=Qf[1];
	if(ferm=="chlepton") qf=Qf[2];
	//deltarhozF()-deltarhowF() is total delta rho 
        k=deltarhozF()-deltarhowF();
...
else {	std::cout<<"wrong input for fermion parameter "<< ferm
        <<"   it can be equal only to 'up','down', 'chlepton'   deltakrem"<<"\n";}
return k;}
...
else {std::cout<<"wrong input for fermion parameter,  it can be equal only to 'up','down', 'chlepton'  uf "<<"\n";};
return uf;
	}

//unnumbered equation after (65)
gslpp::complex EWPOSM::deltarhozF()
{
...
double arg=rzt.arg();
i=gsl_sf_complex_dilog_e(z,arg,&re,&im);
gslpp::complex lterm(re.val-1/6.0*M_PI*M_PI,M_PI*log(1+1/Rz),false);
...
//Equation 67, this is an alternative copy pasted from ZFitter code, I do not understand it...
...
{   gslpp::complex pizg(I3(-(mZ*mZ),mu*mu,mu*mu));
	...
	pizg=(pizg+I3(-(mZ*mZ),mc*mc,mc*mc)+I3(-(mZ*mZ),mt*mt,mt*mt))*2.0*3.0*std::abs(Qf[0])*vf("up");
        //std::cout<<pizg<<"\n";
        pizg=pizg+2.0*3.0*std::abs(Qf[1])*vf("down")*( I3(-(mZ*mZ),md*md,md*md)+I3(-(mZ*mZ),ms*ms,ms*ms)+I3(-(mZ*mZ),mb*mb,mb*mb));
        //std::cout<<pizg<<"\n";
        pizg=pizg+LEPTON*2.0*std::abs(Qf[2])*vf("chlepton")*(I3(-(mZ*mZ),me*me,me*me)+I3(-(mZ*mZ),mmu*mmu,mmu*mmu)+I3(-(mZ*mZ),mtau*mtau,mtau*mtau));
return pizg;}
...
+(9.0/4.0+3.0/4.0/cw2()-3/sw2())*log(cw2())+3.0*rw/4.0/(1.0-rw)*log(rw);
	}
//Eq. 71
...
sig=sig+1/2.0/cw2()*(3.0*(-s/mZ/mZ*(1.0+vf("up")*vf("up"))*i3u+i0u*muq[i]*muq[i]/(mZ*mZ))
...
gslpp::complex XALR(2*log(mW()/mZ),M_PI,false);
...
-LEPTON*(rl/2.0*(1.0-rl*(mW()*mW())*fl)+1.0/6.0/cw2()*(1+vf("chlepton")*vf("chlepton"))*
...
double mlept;
	...
	double rlz=gsl_complex_abs(rhoZf("chlepton"));

	double seff20=sin2thwall("chlepton");//I will have to refer to the sintheata eff form the code
	double seff2=seff20+ale*ale*35.0/18.0*(1-8.0/3.0*kZf("chlepton").real()*sw2());
        //this part comes from the real
        double im_k=kZf("chlepton").real();
        double ratio=(1.0-4.0*std::abs(Qf[2])*(seff2))*(1.0-4.0*std::abs(Qf[2])*(seff2))
        +4*Qf[2]*Qf[2]*seff20*seff20*im_k*im_k;
        g=G0*rlz*sqrt(1-4*mlept*mlept/(mZ*mZ))*((1+2.0*mlept*mlept/(mZ*mZ))*
	(1.0+/* this ratio of couplings*/ratio)
...
{double x;
if((ferm=="up")||(ferm=="down")||(ferm=="chlepton"))
{	if(ferm=="up"){x=1.0-4.0*std::abs(Qf[0])*sin2thwall("up");};
    if(ferm=="down"){x=1.0-4.0*std::abs(Qf[1])*sin2thwall("down");};
    if(ferm=="chlepton"){x=1.0-4.0*std::abs(Qf[2])*sin2thwall("chlepton");}}
else {x=0.0;std::cout<<"wrong input for vf function"<<"\n";};
return x;	}


//Eq.61
gslpp::complex EWPOSM::deltarremtot()
{
gslpp::complex d(deltarrem());
d=d+deltarremQCD();
	return d;}



double EWPOSM::deltar() const
{return 1-GF*mZ*mZ/sqrt(8.0)/M_PI/ale*(4*mW()*mW()/mZ/mZ-4*mW()*mW()*mW()*mW()/mZ/mZ/mZ/mZ);
}


double EWPOSM::rekZf(const std::string& ferm )const
{
    //extracting k from sin^2theta_eff
    double kk=0;
    if((ferm=="up")||(ferm=="down")||ferm=="chlepton")
       {
         kk=sin2thwall(ferm)/sw2();}
       else {kk=0;std::cout
    <<"wrong input for fermion parameter "<< ferm<<"  it can be equal only to 'up','down', 'chlepton'"<<"\n";}
	return kk;}

gslpp::complex EWPOSM::gzf(const std::string& ferm )
{ //formula 2.4.12 of Zfitter paper page80  
...
double qf=0;
    ...
    <<"wrong input for fermion parameter "<< ferm
            <<"  it can be equal only to 'up','down', 'chlepton'"<<"\n";}
 	return k;}

gslpp::complex EWPOSM::gzf1(const std::string& ferm )
{ //formula 2.4.12 of Zfitter paper page80
    gslpp::complex kk(0.0);
...
double qf=0;
	...
	if(ferm=="down") qf=Qf[1];
	if(ferm=="chlepton") qf=Qf[2];
        gslpp::complex xx((1.0-4.0*std::abs(qf)*sin2thwall(ferm)),-4.0*std::abs(qf)*kZf(ferm).imag()*sw2(),false);
        kk=xx;
    }

    else {std::cout<<"wrong input for fermion parameter "
            << ferm<<"  it can be equal only to 'up','down', 'chlepton'"<<"\n";}
    return kk;}


//Equation A.3.41 of ZFitter
gslpp::complex EWPOSM::Deltarho()
{
    gslpp::complex x;
...
gslpp::complex EWPOSM::deltarho()
{
  //  std::cout<<"step 1"<<sw2()/cw2()*SCALE()<<"\n";
   // std::cout<<"step 2"<<ale/4.0/M_PI/sw2()*Deltarho()<<"\n";
    ...
    return ale/4.0/M_PI/sw2()*Deltarho()+sw2()/cw2()*SCALE();
}

double EWPOSM::Gamma_W() const
{return 0.0;}

double EWPOSM::Gamma_Z() const
{return 0.0;}
...
{/*I will try to tranlsate ZFitter code to c, the formulas they are using are different fromt 
 ...
 double VQ=AMZ2/Q2;
  double VQ2=1.0-VQ+VQ*VQ;
   ...
   double   RDI3=-1.0/6.0+(VQ-0.50)/3.0*R1V+VQ2/3.0*R1V*R1V
         -QV*(0.50+R1V)/6.0-VQ*R1V*R1V*((VQ-1.0)/2.0
         +VQ2/3.0*R1V)*log(std::abs(1.0+1.0/AQ));
    XDI3=RDI3;
}
  return XDI3;
 }
...
//I gues this is something like -\Pi_{Z\gamma} bosonic contribution I do not know exaclty it is copied from ZFitter ...
...
double EWPOSM::SCALE()
{ return ale/4/M_PI/sw2()*(41.0/6.0-11.0/3.0*cw2())*log(cw2());
}


/**
 *
 * @return ROQCD eq. 3.4.5 of ZFitter  paper only light quarks
 */
double EWPOSM::ROQCD()
{// for simplicity I set everywhere s=M_z^2
   return -ale*alsMz/M_PI/M_PI/16.0/cw2()/sw2()*(2+vf("up")*vf("up")+vf("down")*vf("down"))*(-1.0) ;}

/**
 *
 * @return AKQCD eq. 3.4.5 of ZFitter paper only light quarks
 */
double EWPOSM::AKQCD()
...
/**
 *
 * @return \delta r_{ud}  from the expression 3.4.4 of ZFItter paper only light quarks
 */
