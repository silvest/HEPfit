...
EWSM::EWSM(const StandardModel& SM_i) : EWModel(), SM(SM_i) {
    ...
    myApproximateFormulae = new ApproximateFormulae(SM);    
}
...
complex EWSM::rhoZ_l_SM(const StandardModel::lepton l) const {    
    ...
    EWSMC->ComputeForNC(Mw_SM());
    
    complex deltaRho_rem_l[orders_EW_size];
    if (flag_order[EW1]) 
        deltaRho_rem_l[EW1] = myOneLoopEW->deltaRho_rem_l(l);
    if (flag_order[EW1QCD1]) 
        deltaRho_rem_l[EW1QCD1] = myTwoLoopQCD->deltaRho_rem_l(l);
    if (flag_order[EW1QCD2]) 
        deltaRho_rem_l[EW1QCD2] = myThreeLoopQCD->deltaRho_rem_l(l);
    if (flag_order[EW2]) 
        deltaRho_rem_l[EW2] = myTwoLoopEW->deltaRho_rem_l(l);
    if (flag_order[EW2QCD1]) 
        deltaRho_rem_l[EW2QCD1] = myThreeLoopEW2QCD->deltaRho_rem_l(l);
    if (flag_order[EW3]) 
        deltaRho_rem_l[EW3] = myThreeLoopEW->deltaRho_rem_l(l);    
    ...
    double deltaRho_rem_l_real[orders_EW_size];
    for (int j=0; j<orders_EW_size; j++)
        deltaRho_rem_l_real[j] = deltaRho_rem_l[j].real();
    double ReRhoZf = resumRhoZ(DeltaRho, deltaRho_rem_l_real, DeltaRbar_rem);

    /* Im[rho_Z^f] without resummation */
    double ImRhoZf = 0.0;
    for (int j=0; j<orders_EW_size; j++)
        ImRhoZf += deltaRho_rem_l[j].imag();    
...
complex EWSM::rhoZ_q_SM(const StandardModel::quark q) const {
    ...
    complex deltaRho_rem_q[orders_EW_size];
    if (flag_order[EW1]) 
        deltaRho_rem_q[EW1] = myOneLoopEW->deltaRho_rem_q(q);
    if (flag_order[EW1QCD1]) 
        deltaRho_rem_q[EW1QCD1] = myTwoLoopQCD->deltaRho_rem_q(q);
    if (flag_order[EW1QCD2]) 
        deltaRho_rem_q[EW1QCD2] = myThreeLoopQCD->deltaRho_rem_q(q);
    if (flag_order[EW2]) 
        deltaRho_rem_q[EW2] = myTwoLoopEW->deltaRho_rem_q(q);
    if (flag_order[EW2QCD1]) 
        deltaRho_rem_q[EW2QCD1] = myThreeLoopEW2QCD->deltaRho_rem_q(q);
    if (flag_order[EW3]) 
        deltaRho_rem_q[EW3] = myThreeLoopEW->deltaRho_rem_q(q);    
    ...
    /* Re[rho_Z^f] with or without resummation */
    double deltaRho_rem_q_real[orders_EW_size];
    for (int j=0; j<orders_EW_size; j++)
        deltaRho_rem_q_real[j] = deltaRho_rem_q[j].real();
    double ReRhoZf = resumRhoZ(DeltaRho, deltaRho_rem_q_real, DeltaRbar_rem);
    
    /* Im[rho_Z^f] without resummation */
    double ImRhoZf = 0.0;
    for (int j=0; j<orders_EW_size; j++)
        ImRhoZf += deltaRho_rem_q[j].imag();    
   
    /* Corrections to the Z-b-bbar vertex */    
    if (q==StandardModel::BOTTOM) {
        throw "Write codes for Zbb in EWSM::rhoZ_q_SM()!";
...
complex EWSM::kappaZ_l_SM(const StandardModel::lepton l) const {    
    ...
    complex deltaKappa_rem_l[orders_EW_size];
    if (flag_order[EW1]) 
        deltaKappa_rem_l[EW1] = myOneLoopEW->deltaKappa_rem_l(l);
    if (flag_order[EW1QCD1]) 
        deltaKappa_rem_l[EW1QCD1] = myTwoLoopQCD->deltaKappa_rem_l(l);
    if (flag_order[EW1QCD2]) 
        deltaKappa_rem_l[EW1QCD2] = myThreeLoopQCD->deltaKappa_rem_l(l);
    if (flag_order[EW2]) 
        deltaKappa_rem_l[EW2] = myTwoLoopEW->deltaKappa_rem_l(l);
    if (flag_order[EW2QCD1]) 
        deltaKappa_rem_l[EW2QCD1] = myThreeLoopEW2QCD->deltaKappa_rem_l(l);
    if (flag_order[EW3]) 
        deltaKappa_rem_l[EW3] = myThreeLoopEW->deltaKappa_rem_l(l);    
    ...
    if (schemeKappaZ==APPROXIMATEFORMULA) {
        ...
        ReKappaZf = myApproximateFormulae->sin2thetaEff(l, DeltaAlpha())/sW2_SM(); 
    } else {
        /* Re[kappa_Z^f] with or without resummation */
        double deltaKappa_rem_l_real[orders_EW_size];
        for (int j=0; j<orders_EW_size; j++)
            deltaKappa_rem_l_real[j] = deltaKappa_rem_l[j].real();
        ReKappaZf = resumKappaZ(DeltaRho, deltaKappa_rem_l_real, DeltaRbar_rem);
    ...
    for (int j=0; j<orders_EW_size; j++)
        ImKappaZf += deltaKappa_rem_l[j].imag();    
...
complex EWSM::kappaZ_q_SM(const StandardModel::quark q) const {
    ...
    complex deltaKappa_rem_q[orders_EW_size];
    if (flag_order[EW1]) 
        deltaKappa_rem_q[EW1] = myOneLoopEW->deltaKappa_rem_q(q);
    if (flag_order[EW1QCD1]) 
        deltaKappa_rem_q[EW1QCD1] = myTwoLoopQCD->deltaKappa_rem_q(q);
    if (flag_order[EW1QCD2]) 
        deltaKappa_rem_q[EW1QCD2] = myThreeLoopQCD->deltaKappa_rem_q(q);
    if (flag_order[EW2]) 
        deltaKappa_rem_q[EW2] = myTwoLoopEW->deltaKappa_rem_q(q);
    if (flag_order[EW2QCD1]) 
        deltaKappa_rem_q[EW2QCD1] = myThreeLoopEW2QCD->deltaKappa_rem_q(q);
    if (flag_order[EW3]) 
        deltaKappa_rem_q[EW3] = myThreeLoopEW->deltaKappa_rem_q(q);    
    ...
    if (schemeKappaZ==APPROXIMATEFORMULA) {
        /* Re[kappa_Z^f] from the approximate formula of 
         * the effective weak mixing angle */
        ReKappaZf = myApproximateFormulae->sin2thetaEff(q, DeltaAlpha())/sW2_SM(); 
    } else {
        /* Re[kappa_Z^f] with or without resummation */
        double deltaKappa_rem_q_real[orders_EW_size];
        for (int j=0; j<orders_EW_size; j++)
            deltaKappa_rem_q_real[j] = deltaKappa_rem_q[j].real();
        ReKappaZf = resumKappaZ(DeltaRho, deltaKappa_rem_q_real, DeltaRbar_rem);    
    ...
    double ImKappaZf = 0.0;
    for (int j=0; j<orders_EW_size; j++)
        ImKappaZf += deltaKappa_rem_q[j].imag();    
    
    /* Corrections to the Z-b-bbar vertex */    
    if (q==StandardModel::BOTTOM) {
        throw "Write codes for Zbb in EWSM::kappaZ_q_SM()!";
...
double EWSM::rho_GammaW_l_SM() const {
    EWSMC->ComputeForRhoWij(Mw_SM());
    double rhoW = 0.0;
    if (flag_order[EW1])
         rhoW = myOneLoopEW->rho_GammaW_l(StandardModel::NEUTRINO_1,StandardModel::ELECTRON);    
    return rhoW;
}


double EWSM::rho_GammaW_q_SM() const {
    EWSMC->ComputeForRhoWij(Mw_SM());
    double rhoW = 0.0;
    if (flag_order[EW1])
         rhoW = myOneLoopEW->rho_GammaW_q(StandardModel::UP,StandardModel::DOWN);    
    return rhoW;
}
...
double EWSM::GammaW_l_SM(const StandardModel::lepton li, 
    ...
    return ( VMNS.abs2()*G0*rho_GammaW_l_SM() );   
}
...
double EWSM::GammaW_q_SM(const StandardModel::quark qi, 
    ...
    double G0 = SM.getGF()*pow(Mw_SM(),3.0)/6.0/sqrt(2.0)/M_PI;    
    return ( 3.0*VCKM.abs2()*G0*rho_GammaW_q_SM()
             *(1.0 + AlsMw/M_PI) );    
}
...
complex EWSM::rhoZ_l_NP_fromSTU(const StandardModel::lepton l, const double T) {
    return ( (1.0 + SM.getAle()*T)*rhoZ_l_SM(l) );
}

complex EWSM::rhoZ_q_NP_fromSTU(const StandardModel::quark q, const double T) {
...
complex EWSM::gZl_over_gAl_NP_fromSTU(const StandardModel::lepton l,
    ...
    double CW2minusSW2 = cW2_SM() - sW2_SM();
    return ( 1.0 - 4.0*fabs(EWSMC->Qf(l))
...
complex EWSM::gZq_over_gAq_NP_fromSTU(const StandardModel::quark q, 
                                      const double S, const double T) {
    double CW2minusSW2 = cW2_SM() - sW2_SM();
    return ( 1.0 - 4.0*fabs(EWSMC->Qf(q))
                   *( 1.0 + SM.getAle()/4.0/sW2_SM()/CW2minusSW2
                            *(S - 4.0*cW2_SM()*sW2_SM()*T) )
