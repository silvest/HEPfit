...
{\bf Step 1:} Model configuration file
...
\item  {\bf A correlation between two observables} can be obtained with: 
...
{\bf Step 2:} Monte Carlo configuration file:
...
* a combined library: libHEPfit.a (installed in `HEPFIT\_INSTALL\_DIR/lib/`
* a combined header file: HEPfit.h (installed in `HEPFIT\_INSTALL\_DIR/include/HEPfit/`)
...
{\bf Use of hepfit-config:}

If `'make install'` has been done, a hepfit-config script can be found in the
`HEPFIT\_INSTALL\_DIR/bin/` directory, which can be invoked with the 
following options:
