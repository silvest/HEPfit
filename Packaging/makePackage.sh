...
do
    if [ ! -d "${SRCDIR}/${PROJECT}" ]; then
        echo "midir ${SRCDIR}/${PROJECT}"
        mkdir ${SRCDIR}/${PROJECT}
    fi

    if [ "$PROJECT" = "Analysis" ]; then
        cp -af ${ORGDIR}/${PROJECT}/main.cpp ${SRCDIR}/${PROJECT}/
    else
        cp -af ${ORGDIR}/${PROJECT}/src ${SRCDIR}/${PROJECT}/
    fi
done

if [ ! -d "${SRCDIR}/doc" ]; then
    echo "mkdir ${SRCDIR}/doc"
    mkdir ${SRCDIR}/doc
fi
cp -af ${ORGDIR}/doc/COPYING ${SRCDIR}/doc/
cp -af ${ORGDIR}/doc/LICENSE ${SRCDIR}/doc/

###########################################################
# remove unnecessary files

rm -f ${SRCDIR}/StandardModel/src/EWSMTwoFermionsLEP2.h
rm -f ${SRCDIR}/StandardModel/src/EWSMTwoFermionsLEP2.cpp
rm -f ${SRCDIR}/StandardModel/src/EWSM_Output.h
rm -f ${SRCDIR}/StandardModel/src/EWSM_Output.cpp
rm -f ${SRCDIR}/EW/src/LEP2*
...
#do
#    rm -f ${SRCDIR}/Flavour/src/${FLAVOR}
#done
...
do
    ...
    python3 $PYSCRIPT $PROJECT $CONF > ${SRCDIR}/${PROJECT}/CMakeLists.txt
done
...
cp ${SCRIPTPATH}/etc/CMakeLists.txt ${SRCDIR}/
cp ${SCRIPTPATH}/etc/SusyFit_noMCMC.h.in ${SRCDIR}/
cp ${SCRIPTPATH}/etc/BAT_make_wrapper.sh.in ${SRCDIR}/
cp ${SCRIPTPATH}/etc/cmake_uninstall.cmake.in ${SRCDIR}/
cp ${SCRIPTPATH}/etc/susyfit-config.in ${SRCDIR}/
eval sed "$SED_ARG" ${SCRIPTPATH}/etc/SusyFit.pc.in > ${SRCDIR}/SusyFit.pc.in
cp ${SCRIPTPATH}/etc/SusyFit.h.in ${SRCDIR}/
cp ${SCRIPTPATH}/etc/BAT_mpi_patch.txt ${SRCDIR}/
...
sh ${SCRIPTPATH}/mod.sh ${SRCDIR}/InputParser/src/ModelFactory.cpp ${SRCDIR}/EW/src/EWObservables.h ${SRCDIR}/InputParser/src/ThObsFactory.cpp ${SRCDIR}/StandardModel/src/StandardModel.cpp ${SRCDIR}/StandardModel/src/StandardModel.h
