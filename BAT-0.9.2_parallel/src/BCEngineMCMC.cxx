...
void BCEngineMCMC::MCMCGetNewPointMetropolis(int parameter) {
    int mychain = 0, iproc = 0;
    int buffsize = fMCMCxLocal.size() + 1;
    double p0[procnum];
    int indchain[procnum];
...
//        std::cout << "begin of while: chain " << mychain << std::endl;
        ...
        // get proposal point
        if (!MCMCGetProposalPointMetropolis(mychain, parameter, fMCMCxLocal)) {
            // increase counter
            fMCMCNTrialsFalse[mychain * fMCMCNParameters + parameter]++;
