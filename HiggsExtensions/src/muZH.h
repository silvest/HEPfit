...
public:
     ...
     */
    muZH(const HiggsExtensionModel& HESM_i) : ThObservable(HESM_i), HESM(static_cast<const HiggsExtensionModel&> (HESM_i))
    {
        if(HESM_i.ModelName().compare(0,5,"Higgs")!=0)
            throw std::runtime_error("ERROR: the muZH constructor can only be used with a HiggsExtensionModel reference, while I got " +
